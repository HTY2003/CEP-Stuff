...
Not added to the splay tree ADT to maintain versatility of the original ADT.
    ...
    def bocount(self, max):
        ...
        for node in self._inOrderGen(self.root):
            if node[0]< max: count += 1
        return count

    def boprint(self, min, max):
        for node in self._inOrderGen(self.root):
            if node[0] > min and node[0] < max: print(node) #note: min and max are included in the printed list
...
Not added to the splay tree ADT to maintain versatility of the original ADT.
    ...
    def scoreprint(self, min=-1, max=101):
        for node in self._inOrderGen(self.root):
            if node[0] > min and node[0] < max: print(node) #note: min and max are not included in the printed list
